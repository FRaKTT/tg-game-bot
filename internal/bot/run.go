...
func (b *Bot) handleAdminMessage(msg *tgbotapi.Message) error { //nolint:gocognit //todo
	var adminButtons = []string{adminButtonStorage, adminButtonLogs}

	if b.adminChatID == 0 {
		b.adminChatID = msg.Chat.ID // save for async notifications
	}
	...
	case adminButtonLogs: // прочитать логи
		...
		const nRowsInTextPiece = 20
		textPieces := textRowsToPieces(text, nRowsInTextPiece)
...
func (b *Bot) handleUnknownUserMessage(msg *tgbotapi.Message) error {
	...
	alertToAdminMsg := fillResponseMsg(b.adminChatID, unknownUserMsg, nil)
...
// todo: правильнее разбивать по кол-ву символов (4096, check)
//
//	и/или уменьшать размер при получении ошибки MESSAGE_TOO_LONG (https://core.telegram.org/method/messages.sendMessage)
